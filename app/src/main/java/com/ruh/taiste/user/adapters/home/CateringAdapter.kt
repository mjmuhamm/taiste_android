...
class CateringAdapter(private val context: Context, private var caterItems: MutableList<FeedMenuItems>) : RecyclerView.Adapter<CateringAdapter.ViewHolder>()  {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (item.liked.contains(FirebaseAuth.getInstance().currentUser!!.email!!)) {
        ...
        holder.likeButton.setOnClickListener {
            ...
            if (holder.likeButton.isSelected) {
                ...
                holder.likeText.text = "${holder.likeText.text.toString().toInt() - 1}"
                db.collection("User").document(FirebaseAuth.getInstance().currentUser!!.uid).collection("UserLikes").document(item.menuItemId).delete()
                val data: Map<String, Any> = hashMapOf("liked" to FieldValue.arrayRemove(FirebaseAuth.getInstance().currentUser!!.email!!))
                db.collection(item.itemType).document(item.menuItemId).update(data)

            }
            val docRef = db.collection("User").document(FirebaseAuth.getInstance().currentUser!!.uid).collection("UserChefs").document(item.chefEmail)
    ...
    class ViewHolder(itemView: MenuFeedPostBinding) : RecyclerView.ViewHolder(itemView.root) {
        ...
        fun bind(item: FeedMenuItems) {
            ...
            chefRef.child("chefs/${item.chefEmail}/Cater Items/${item.menuItemId}0.png").downloadUrl.addOnSuccessListener { itemUri ->
